...
@php
    $defaultItemType = $defaultItemType ?? 'Raw Material';
    $pageTitle = $pageTitle ?? 'Item Master';
    $pageDescription = $pageDescription ?? 'Manage raw materials, consumables, and finished goods.';
@endphp

@section('title', $pageTitle.' - Timber Inventory')
...
@section('content')
    <section class="erp-card item-list-card">
        ...
        <div class="item-bucket-row" id="item-bucket-row" hidden>
            <button class="item-bucket-btn is-active" type="button" data-item-bucket="Raw Material">
                <span>Raw Material</span>
                <small>Raw material CRUD</small>
            </button>
            <button class="item-bucket-btn" type="button" data-item-bucket="Finish Product">
                <span>Finish Product</span>
                <small>Finished product CRUD</small>
            </button>
        </div>

        <div class="item-filter-row" id="item-filter-row" hidden>
            <div class="item-filter-group">
                <label class="item-filter-field" hidden>
                    <span>Item Type</span>
                    <select id="filter-type"><option value="">All Types</option><option>Raw Material</option><option>Finish Product</option></select>
                </label>
        ...
        <div class="table-toolbar item-table-toolbar">
            ...
            <div class="module-search">
                ...
                <input id="search" type="search" placeholder="Search item code, name, type, category">
            ...
            <table class="table item-data-table">
                <thead>
                    ...
                    <tr class="item-filter-head" id="item-filter-head" hidden>
                        ...
                        <th data-col="item_type"><select id="col-type"><option value="">All</option><option>Raw Material</option><option>Finish Product</option></select></th>
                        ...
                        <th data-col="category"><input id="col-category" type="search" placeholder="Search"></th>
...
<style>
    ...
    .item-bucket-row { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:12px; padding:16px 20px 0; }
    .item-bucket-btn { display:grid; gap:4px; padding:14px 16px; border:1px solid var(--border); border-radius:16px; background:linear-gradient(180deg, color-mix(in srgb, var(--surface-soft) 42%, var(--surface)), var(--surface)); color:var(--text); text-align:left; transition:transform .16s ease, box-shadow .16s ease, border-color .16s ease; }
    .item-bucket-btn:hover { transform:translateY(-1px); box-shadow:0 10px 22px rgba(15,23,42,.08); }
    .item-bucket-btn span { font-size:15px; font-weight:900; }
    .item-bucket-btn small { color:var(--muted); font-size:12px; font-weight:700; }
    .item-bucket-btn.is-active { border-color:var(--primary); box-shadow:0 10px 22px rgba(var(--primary-rgb), .12); }
    ...
    .item-data-table { min-width:1400px; table-layout:auto; border-collapse:separate; border-spacing:0; }
    ...
    .item-form-grid .field-wide { grid-column:1 / -1; width:100%; }
    .item-type-switch { display:grid; gap:10px; padding:14px; border:1px solid var(--border); border-radius:var(--radius); background:linear-gradient(180deg, color-mix(in srgb, var(--surface-soft) 60%, var(--surface)), var(--surface)); }
    .item-type-switch-head h3 { margin:0; font-size:14px; font-weight:900; text-transform:uppercase; letter-spacing:.04em; }
    .item-type-switch-head p { margin:4px 0 0; font-size:12px; }
    .item-type-switch-body { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px; }
    .item-type-pill { min-height:42px; padding:10px 14px; border:1px solid var(--border); border-radius:999px; background:var(--surface); color:var(--text); font-weight:800; text-align:center; transition:transform .16s ease, border-color .16s ease, background .16s ease, color .16s ease; }
    .item-type-pill:hover { transform:translateY(-1px); border-color:color-mix(in srgb, var(--primary) 32%, var(--border)); }
    .item-type-pill.is-active { color:#fff; border-color:var(--primary); background:var(--primary); box-shadow:0 8px 18px rgba(var(--primary-rgb), .22); }
    @media (max-width:900px) { .item-form-grid { grid-template-columns:1fr; } }
    @media (max-width:900px) { .item-form-grid .field-wide { grid-column:span 1; } }
    @media (max-width:640px) { .item-type-switch-body { grid-template-columns:1fr; } }
    @media (max-width:768px) {
        .item-card-head, .item-filter-row, .item-table-toolbar, .pagination-row { align-items:stretch; flex-direction:column; }
        .item-bucket-row { grid-template-columns:1fr; padding-inline:12px; }
        ...
        .item-data-table { min-width:1340px; }
    }
</style>
<script>
...
const columnOptions=[['item_code','Code'],['item_name','Name'],['item_type','Type'],['material_type_id','Material'],['uom_id','UOM'],['length_mm','Length'],['width_mm','Width'],['thickness_mm','Thickness'],['minimum_stock','Min Stock'],['category','Category'],['opening_qty','Opening'],['status','Status']];
const defaultVisibleColumns=['item_code','item_name','item_type','material_type_id','uom_id','minimum_stock','category','status'];
const columnStorageKey='items.visibleColumns';
const itemTypeChoices=['Raw Material','Finish Product'];
const pageDefaultItemType=@json($defaultItemType);
let page=1,last=1,timer,records=[],materialTypes=[],uoms=[],sortBy=sessionStorage.getItem('items.sortBy.v2')||'created_at',sortDirection=sessionStorage.getItem('items.sortDirection.v2')||'desc',visibleColumns=readVisibleColumns(),activeId=null,activeMode='create',paginationMeta={from:0,to:0,total:0},activeItemBucket=pageDefaultItemType;
...
const typePills=(selected,readonly=false)=>itemTypeChoices.map(type=>`<button class="item-type-pill ${selected===type?'is-active':''}" type="button" data-item-type="${type}" aria-pressed="${selected===type?'true':'false'}" ${readonly?'disabled':''}>${type}</button>`).join('');
...
function setModalItemType(type){const value=itemTypeChoices.includes(type)?type:'Raw Material';const hidden=document.getElementById('modal-type');if(hidden)hidden.value=value;document.querySelectorAll('[data-item-type]').forEach(button=>{const active=button.dataset.itemType===value;button.classList.toggle('is-active',active);button.setAttribute('aria-pressed',active?'true':'false')})}
function setItemBucket(type){activeItemBucket=itemTypeChoices.includes(type)?type:'Raw Material';sessionStorage.setItem('items.bucket',activeItemBucket);filterType.value=activeItemBucket;document.querySelectorAll('[data-item-bucket]').forEach(button=>button.classList.toggle('is-active',button.dataset.itemBucket===activeItemBucket));}
async function boot(){document.querySelectorAll('[data-manage-only]').forEach(el=>el.hidden=!canManageItems);[materialTypes,uoms]=await Promise.all([lookup('/v1/material-types'),lookup('/v1/uoms')]);filterMaterial.innerHTML=opt(materialTypes,'material_type_id','material_type_name','All Materials');colMaterial.innerHTML=opt(materialTypes,'material_type_id','material_type_name','All');filterUom.innerHTML=opt(uoms,'uom_id','uom_name','All UOMs');colUom.innerHTML=opt(uoms,'uom_id','uom_name','All');renderColumnPicker();restoreState();applyColumnVisibility();const bucketRow=document.getElementById('item-bucket-row');if(bucketRow)bucketRow.hidden=true;const typeField=filterType?.closest('.item-filter-field');if(typeField)typeField.hidden=true;load()}
function params(){return{page,per_page:perPage.value,search:search.value,item_type:activeItemBucket,material_type_id:colMaterial.value||filterMaterial.value,uom_id:colUom.value||filterUom.value,status:colStatus.value||filterStatus.value,item_code:colCode.value,item_name:colName.value,length_min:colLengthMin.value,width_min:colWidthMin.value,thickness_min:colThicknessMin.value,minimum_stock_min:colMinStock.value,category:colCategory.value,opening_qty_min:colOpeningMin.value,sort_by:sortBy,sort_direction:sortDirection}}
async function load(p=page){page=p;storeState();rows.innerHTML='<tr class="skeleton-row"><td colspan="13"></td></tr>';const r=await axios.get(endpoint,{params:params()});const d=r.data.data;page=d.current_page;last=d.last_page;paginationMeta={from:d.from||0,to:d.to||0,total:d.total||0};records=d.data||[];renderRows();renderSortHeaders();renderPager()}
function badge(status){return`<span class="badge ${status==='Active'?'badge-success':'badge-muted'}">${esc(status||'Active')}</span>`}
function renderRows(){rows.innerHTML=records.length?records.map(x=>`<tr><td data-col="item_code"><strong>${esc(x.item_code)}</strong></td><td data-col="item_name">${esc(x.item_name)}</td><td data-col="item_type">${esc(x.item_type)}</td><td data-col="material_type_id">${esc(materialName(x.material_type_id))}</td><td data-col="uom_id">${esc(uomName(x.uom_id))}</td><td data-col="length_mm" class="text-end">${num(x.length_mm)}</td><td data-col="width_mm" class="text-end">${num(x.width_mm)}</td><td data-col="thickness_mm" class="text-end">${num(x.thickness_mm)}</td><td data-col="minimum_stock" class="text-end">${num(x.minimum_stock)}</td><td data-col="category">${esc(x.category)}</td><td data-col="opening_qty" class="text-end">${num(x.opening_qty)}</td><td data-col="status">${badge(x.status)}</td><td class="text-end action-col"><span class="item-actions"><button class="icon-btn action-view" data-view="${x.item_id}" title="View" aria-label="View"><i data-lucide="eye"></i></button>${canManageItems?`<button class="icon-btn action-edit" data-edit="${x.item_id}" title="Edit" aria-label="Edit"><i data-lucide="pencil"></i></button><button class="icon-btn action-delete" data-delete="${x.item_id}" title="Delete" aria-label="Delete"><i data-lucide="trash-2"></i></button>`:''}</span></td></tr>`).join(''):'<tr><td colspan="13" class="text-muted">No item records found.</td></tr>';applyColumnVisibility();lucide?.createIcons()}
...
function storeState(){sessionStorage.setItem('items.search',search.value);sessionStorage.setItem('items.type',activeItemBucket);sessionStorage.setItem('items.material',filterMaterial.value);sessionStorage.setItem('items.uom',filterUom.value);sessionStorage.setItem('items.status',filterStatus.value);sessionStorage.setItem('items.perPage',perPage.value);sessionStorage.setItem('items.sortBy.v2',sortBy);sessionStorage.setItem('items.sortDirection.v2',sortDirection);sessionStorage.setItem('items.bucket',activeItemBucket);['col-code','col-name','col-type','col-material','col-uom','col-length-min','col-width-min','col-thickness-min','col-min-stock','col-category','col-opening-min','col-status'].forEach(id=>sessionStorage.setItem(`items.${id}`,document.getElementById(id).value))}
function restoreState(){search.value=sessionStorage.getItem('items.search')||'';setItemBucket(pageDefaultItemType);filterMaterial.value=sessionStorage.getItem('items.material')||'';filterUom.value=sessionStorage.getItem('items.uom')||'';filterStatus.value=sessionStorage.getItem('items.status')||'';perPage.value='10';['col-code','col-name','col-type','col-material','col-uom','col-length-min','col-width-min','col-thickness-min','col-min-stock','col-category','col-opening-min','col-status'].forEach(id=>{const el=document.getElementById(id);el.value=sessionStorage.getItem(`items.${id}`)||''})}
function resetFilters(){[search,filterType,filterMaterial,filterUom,filterStatus,colCode,colName,colType,colMaterial,colUom,colLengthMin,colWidthMin,colThicknessMin,colMinStock,colCategory,colOpeningMin,colStatus].forEach(el=>el.value='');perPage.value='10';sortBy='created_at';sortDirection='desc';load(1)}
function modalBody(readonly=false){
return`<div class="item-modal-body">
<input id="modal-type" type="hidden" value="Raw Material">
<div class="item-form-grid">
<div class="field">
<i data-lucide="hash">
</i><label>Item Code</label>
<input id="modal-code" type="text" ${readonly?'disabled':''}>
</div><div class="field"><i data-lucide="package"></i>
<label>Item Name *</label><input id="modal-name" type="text" ${readonly?'disabled':''}></div>
<div class="field"><i data-lucide="boxes">
</i><label>Material Type</label><select id="modal-material" ${readonly?'disabled':''}>${opt(materialTypes,'material_type_id','material_type_name','Material')}</select></div>
<div class="field"><i data-lucide="ruler">
</i><label>UOM</label><select id="modal-uom" ${readonly?'disabled':''}>${opt(uoms,'uom_id','uom_name','UOM')}</select>
</div><div class="field">
<i data-lucide="activity"></i>
<label>Status</label>
<select id="modal-status" ${readonly?'disabled':''}><option>Active</option>
<option>Inactive</option></select></div><div class="field">
<i data-lucide="move-horizontal"></i><label>Length MM</label>
<input id="modal-length" type="number" step="0.001" min="0" ${readonly?'disabled':''}></div>
<div class="field"><i data-lucide="move-horizontal"></i><label>Width MM</label>
<input id="modal-width" type="number" step="0.001" min="0" ${readonly?'disabled':''}></div>
<div class="field"><i data-lucide="move-vertical"></i><label>Thickness MM</label>
<input id="modal-thickness" type="number" step="0.001" min="0" ${readonly?'disabled':''}></div>
<div class="field"><i data-lucide="calculator"></i><label>CFT Factor</label>
<input id="modal-cft" type="number" step="0.001" min="0" ${readonly?'disabled':''}></div>
<div class="field"><i data-lucide="warehouse"></i><label>Minimum Stock</label>
<input id="modal-min-stock" type="number" step="0.001" min="0" ${readonly?'disabled':''}></div>
<div class="field"><i data-lucide="tags"></i><label>Category</label
><input id="modal-category" type="text" ${readonly?'disabled':''}></div></div>
<div id="modal-errors" class="form-errors" hidden></div></div>`}
function fillModal(row){document.getElementById('modal-code').value=row.item_code||'';document.getElementById('modal-name').value=row.item_name||'';setModalItemType(row.item_type||'Raw Material');document.getElementById('modal-material').value=row.material_type_id||'';document.getElementById('modal-uom').value=row.uom_id||'';document.getElementById('modal-status').value=row.status||'Active';document.getElementById('modal-length').value=row.length_mm||'';document.getElementById('modal-width').value=row.width_mm||'';document.getElementById('modal-thickness').value=row.thickness_mm||'';document.getElementById('modal-cft').value=row.cft_factor||'';document.getElementById('modal-min-stock').value=row.minimum_stock||'';document.getElementById('modal-category').value=row.category||''}
async function openItemModal(mode,id=null){activeMode=mode;activeId=id;const readonly=mode==='view';window.ErpModal.open({title:mode==='create'?'Add Item':mode==='edit'?'Edit Item':'Display Item',subtitle:'Item master details and stock defaults',size:'xl',body:modalBody(readonly),footer:readonly?'<button class="btn-erp" type="button" data-modal-close><i data-lucide="x"></i> Close</button>':'<button class="btn-erp" type="button" data-modal-close><i data-lucide="x"></i> Cancel</button><button class="btn-erp btn-primary" type="button" data-action="save-item"><i data-lucide="save"></i> Save</button>'});if(mode==='create'){fillModal({status:'Active',item_type:activeItemBucket,minimum_stock:0});lucide?.createIcons();return}const r=await axios.get(`${endpoint}/${id}`);fillModal(r.data.data);if(r.data.data?.item_type) setItemBucket(r.data.data.item_type);lucide?.createIcons()}
function payload(){return{item_code:document.getElementById('modal-code').value,item_name:document.getElementById('modal-name').value,item_type:document.getElementById('modal-type').value,material_type_id:document.getElementById('modal-material').value,uom_id:document.getElementById('modal-uom').value,length_mm:document.getElementById('modal-length').value,width_mm:document.getElementById('modal-width').value,thickness_mm:document.getElementById('modal-thickness').value,cft_factor:document.getElementById('modal-cft').value,minimum_stock:document.getElementById('modal-min-stock').value,category:document.getElementById('modal-category').value,status:document.getElementById('modal-status').value}}
...
function downloadImportTemplate(){const csvRows=[['item_code','item_name','item_type','material_type','uom','length_mm','width_mm','thickness_mm','cft_factor','minimum_stock','category','status'],['RM-PLY-001','Plywood 18mm','Raw Material','Wood','CFT','2440','1220','18','1','10','Sheet','Active']];if(activeItemBucket==='Finish Product')csvRows[1]=['FG-PALLET-001','Standard Pallet','Finish Product','Finished Goods','PCS','1200','1000','150','1','0','Packed Goods','Active'];const csv='\uFEFF'+csvRows.map(row=>row.map(value=>`"${String(value).replaceAll('"','""')}"`).join(',')).join('\r\n');const blob=new Blob([csv],{type:'text/csv;charset=utf-8'});const link=document.createElement('a');const url=URL.createObjectURL(blob);link.href=url;link.download=`${activeItemBucket.toLowerCase().replaceAll(' ','-')}-item-import-template.csv`;link.style.display='none';document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);window.ErpToast?.show('Sample file downloaded.')}
function openImportModal(){window.ErpModal.open({title:`Import ${activeItemBucket} Items`,subtitle:`Upload bulk ${activeItemBucket.toLowerCase()} records from the sample file`,size:'md',body:`<div class="item-import-body"><p class="text-muted">First download the sample file for ${activeItemBucket.toLowerCase()}, fill item rows, then upload the saved CSV. Existing item codes will be updated.</p><div class="field"><i data-lucide="file-spreadsheet"></i><label>CSV File *</label><input id="item-import-file" type="file" accept=".csv,text/csv"></div><div id="item-import-errors" class="form-errors" hidden></div></div>`,footer:`<button class="btn-erp" type="button" data-action="download-item-sample"><i data-lucide="download"></i> Sample ${activeItemBucket}</button><button class="btn-erp" type="button" data-modal-close><i data-lucide="x"></i> Cancel</button><button class="btn-erp btn-primary" type="button" data-action="upload-item-import"><i data-lucide="upload"></i> Upload</button>`});lucide?.createIcons()}
