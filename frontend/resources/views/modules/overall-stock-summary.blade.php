...
@section('title', 'OverAll Stock Summary - Timber Inventory')
...
async function boot(){const [items,types]=await Promise.all([
    lookup('/v1/items'),
    lookup('/v1/material-types')
]);

item_id.innerHTML = opts(items,'item_id','item_name','All Items');
item_type.innerHTML = itemTypeOptions(items);
material_type_id.innerHTML = opts(types,'material_type_id','material_type_name','All Types');
colMaterialType.innerHTML=opts(types,'material_type_id','material_type_name','All');
renderColumnPicker();applyColumnVisibility();load()}
...
function params(){storeSort();return{page,per_page:per_page.value,search:search.value,item_id:item_id.value,item_type:item_type.value,material_type_id:colMaterialType.value||material_type_id.value,sort_by:sortBy,sort_direction:sortDirection}}
function clientFiltered(rows) {

    const code = (colItemCode?.value || '').toLowerCase();
    const name = (colItemName?.value || '').toLowerCase();
    const min = Number(colQtyMin?.value || 0);
    const date = colLastMovement?.value || '';
    const status = colStatus?.value || '';

    return rows.filter(x =>
        (!code || String(x.item_code || '').toLowerCase().includes(code)) &&
        (!name || String(x.item_name || '').toLowerCase().includes(name)) &&
        (!colQtyMin?.value || Number(x.available_qty || 0) >= min) &&
        (!date || String(x.last_movement_date || '').slice(0, 10) === date) &&
        (!status || stockStatus(x.available_qty, x.minimum_stock) === status)
    );
}
...
async function load(p = page) {
    ...
    rows.innerHTML = '<tr class="skeleton-row"><td colspan="12"></td></tr>';

    const r = await axios.get(endpoint, { params: params() });
    ...
    last = d.last_page || 1;
    paginationMeta = { from: d.from || 0, to: d.to || 0, total: d.total || 0 };
    m_total.textContent = m.total_items ?? 0;
    m_stock.textContent = m.available_stock ?? 0;
    m_low.textContent = m.low_stock_items ?? 0;
    m_out.textContent = m.out_of_stock_items ?? 0;
    summaryRows = clientFiltered(raw);

    // ❌ IMPORTANT: avoid filtering before DOM is ready
    renderRows();
    renderSortHeaders();
    renderPager();
}
...
function resetFilters(){[search,item_id,item_type,material_type_id,colItemCode,colItemName,colMaterialType,colQtyMin,colLastMovement,colStatus].forEach(el=>el.value='');per_page.value='10';sortBy='item_name';sortDirection='asc';load(1)}
function csvExport() {
    ...
    const valueMap = {
        ...
        uom: x => x.uom_name,
        stock_type: x => x.stock_type || 'Fresh',
        available_qty: x => x.available_qty,
        rate: x => x.avg_rate,
        value: x => x.stock_value,
        last_movement: x => x.last_movement_date,
        status: x => stockStatus(x.available_qty, x.minimum_stock)
    };

    const activeColumns = columnOptions
        .filter(([key]) => visibleColumns.includes(key))
        .filter(([key]) => valueMap[key]); // safety guard
    ...
    const header = activeColumns.map(([, label]) => esc(label)).join(',');

    const rowsCsv = summaryRows.map(row =>
        activeColumns
            .map(([key]) => {
                const fn = valueMap[key];
                return esc(fn ? fn(row) : '');
            })
            .join(',')
    );
    ...
    a.href = url;
    a.download = `overall-stock-summary-${Date.now()}.csv`;
