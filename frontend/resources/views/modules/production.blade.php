...
<style>
    ...
    .production-form-grid .field-time { grid-column:span 1; }
    .production-form-grid .field-cost { grid-column:span 1; }
    .production-line-wrap { margin:0; }
    .production-line-wrap .table { min-width:1320px; }
    ...
    .line-actions { display:inline-flex; justify-content:flex-end; width:100%; }
    .line-actions .icon-btn { width:30px; height:30px; border-radius:6px; }
    .line-actions .icon-btn svg { width:16px; height:16px; }
...
function defaultRmLocationId(){return locations.find(location=>String(location.location_type||'').toUpperCase()==='RM')?.location_id||''}
function defaultConsumptionLocationId(){return defaultRmLocationId()||locations[0]?.location_id||''}
function uomForItem(itemId){return items.find(item=>String(item.item_id)===String(itemId))?.uom_id||''}
function setRowDefaults(tr,forceUom=false){const item=tr.querySelector('[name=item_id]'),uom=tr.querySelector('[name=uom_id]'),location=tr.querySelector('[name=location_id]');if(item&&uom&&(forceUom||!uom.value))uom.value=uomForItem(item.value);if(location&&!location.value)location.value=defaultConsumptionLocationId()}
function productionModalBody(readonly=false){return`<div class="production-modal-body"><div class="production-status-strip"><span id="modal-status-label">Draft</span><span id="modal-reference-label"></span></div><div class="production-form-grid"><div class="field"><i data-lucide="hash"></i><label>Production No</label><input id="modal-production-no" type="text" ${readonly?'disabled':'readonly'}></div><div class="field"><i data-lucide="calendar"></i><label>Date *</label><input id="modal-production-date" type="date" ${readonly?'disabled':''}></div><div class="field field-time"><i data-lucide="clock-3"></i><label>Time *</label><input id="modal-production-time" type="time" ${readonly?'disabled':''}></div><div class="field"><i data-lucide="workflow"></i><label>BOM *</label><select id="modal-bom-id" ${readonly?'disabled':''}>${opt(boms,'bom_id','bom_name','Select BOM')}</select></div><div class="field"><i data-lucide="package-check"></i><label>Produced Item *</label><select id="modal-produced-item-id" disabled>${opt(items,'item_id','item_name','Produced Item')}</select></div><div class="field"><i data-lucide="users"></i><label>Team *</label><select id="modal-team-id" ${readonly?'disabled':''}>${opt(teams,'team_id','team_name','Team')}</select></div><div class="field"><i data-lucide="map-pin"></i><label>FG Location *</label><select id="modal-fg-location-id" ${readonly?'disabled':''}>${opt(locations,'location_id','location_name','FG Location')}</select></div><div class="field"><i data-lucide="scale"></i><label>Produced Qty *</label><input id="modal-produced-qty" type="number" step="0.001" min="0.001" value="1" ${readonly?'disabled':''}></div><div class="field field-cost"><i data-lucide="indian-rupee"></i><label>Cost</label><input id="modal-production-cost" type="number" step="0.01" min="0" value="0" ${readonly?'disabled':''}></div><div class="field field-cost"><i data-lucide="indian-rupee"></i><label>Labour Charge</label><input id="modal-labour-charge" type="number" step="0.01" min="0" value="0" ${readonly?'disabled':''}></div><div class="field field-wide"><i data-lucide="message-square"></i><label>Remarks</label><input id="modal-remarks" type="text" ${readonly?'disabled':''}></div></div><div class="table-toolbar" style="padding:0;"><h3 class="erp-card-title">Material Consumption</h3>${readonly?'':'<div class="module-actions"><button class="btn-erp" type="button" data-action="add-consumption-line"><i data-lucide="plus"></i> Add Line</button><button class="btn-erp" type="button" data-action="load-bom-lines"><i data-lucide="refresh-cw"></i> Load BOM</button></div>'}</div><div class="table-responsive production-line-wrap"><table class="table"><thead><tr><th>Item</th><th>UOM</th><th>Location</th><th class="text-end">Current Stock</th><th class="text-end">Required</th><th class="text-end">Consumed</th><th class="text-end">Wastage</th><th>Remarks</th><th class="text-end">Actions</th></tr></thead><tbody id="modal-consumption-rows"></tbody></table></div><div id="modal-audit-section" hidden><div class="table-toolbar" style="padding:0;"><h3 class="erp-card-title">Audit Trail</h3></div><div id="modal-audit-grid"></div></div><div id="modal-errors" class="form-errors" hidden></div></div>`}
...
function consumptionLine(row={},readonly=false){const tr=document.createElement('tr');tr.innerHTML=`<td><select class="line-select" name="item_id" required ${readonly?'disabled':''}>${opt(items,'item_id','item_name','Item')}</select></td><td><select class="line-select" name="uom_id" ${readonly?'disabled':''}>${opt(uoms,'uom_id','uom_name','UOM')}</select></td><td><select class="line-select" name="location_id" required ${readonly?'disabled':''}>${opt(locations,'location_id','location_name','Location')}</select></td><td class="text-end" data-current-stock>${num(row.current_stock)}</td><td><input class="line-input line-number" name="required_qty" type="number" step="0.001" min="0" value="${row.required_qty??0}" ${readonly?'disabled':''}></td><td><input class="line-input line-number" name="consumed_qty" type="number" step="0.001" min="0.001" value="${row.consumed_qty??row.required_qty??1}" required ${readonly?'disabled':''}></td><td><input class="line-input line-number" name="wastage_qty" type="number" step="0.001" min="0" value="${row.wastage_qty??Math.max(Number(row.consumed_qty||0)-Number(row.required_qty||0),0)}" ${readonly?'disabled':''}></td><td><input class="line-input" name="remarks" value="${esc(row.remarks)}" ${readonly?'disabled':''}></td><td class="text-end">${readonly?'':`<div class="line-actions"><button class="icon-btn action-delete" type="button" data-remove-consumption-line title="Remove"><i data-lucide="trash-2"></i></button></div>`}</td>`;document.getElementById('modal-consumption-rows').appendChild(tr);tr.querySelector('[name=item_id]').value=row.item_id||'';tr.querySelector('[name=uom_id]').value=row.uom_id||uomForItem(row.item_id)||'';tr.querySelector('[name=location_id]').value=row.location_id||defaultConsumptionLocationId()||'';if(!readonly){tr.querySelector('[name=item_id]').addEventListener('change',()=>{setRowDefaults(tr,true);refreshLineStock(tr)});tr.querySelector('[name=location_id]').addEventListener('change',()=>refreshLineStock(tr));}setRowDefaults(tr);refreshLineStock(tr)}
...
async function loadProductionEditorForCreate(){showProductionEditor('create');document.getElementById('modal-status-label').textContent='Draft';document.getElementById('modal-reference-label').textContent='';document.getElementById('modal-production-date').value=new Date().toISOString().slice(0,10);document.getElementById('modal-production-time').value=localTimeValue();document.getElementById('modal-produced-qty').value=1;document.getElementById('modal-production-cost').value=0;document.getElementById('modal-labour-charge').value=0;document.getElementById('modal-remarks').value='';document.getElementById('modal-audit-section').hidden=true;document.getElementById('modal-audit-grid').innerHTML='';document.getElementById('modal-consumption-rows').innerHTML='';const r=await axios.get(endpoint+'/next-number');document.getElementById('modal-production-no').value=r.data.data.production_no;consumptionLine({},false)}
async function loadProductionEditorForEdit(id){activeId=id;showProductionEditor('edit');const r=await axios.get(`${endpoint}/${id}`);const x=r.data.data;document.getElementById('modal-status-label').innerHTML=badge(x.status);document.getElementById('modal-reference-label').textContent=x.posted_at?`Posted: ${x.posted_at}`:x.cancelled_at?`Cancelled: ${x.cancelled_at}`:'';document.getElementById('modal-production-no').value=x.production_no;document.getElementById('modal-production-date').value=x.production_date;document.getElementById('modal-production-time').value=(x.production_time||'').slice(0,5);document.getElementById('modal-bom-id').value=x.bom_id;document.getElementById('modal-produced-item-id').value=x.produced_item_id;document.getElementById('modal-team-id').value=x.team_id;document.getElementById('modal-fg-location-id').value=x.fg_location_id;document.getElementById('modal-produced-qty').value=x.produced_qty;document.getElementById('modal-production-cost').value=x.production_cost;document.getElementById('modal-labour-charge').value=x.labour_charge||0;document.getElementById('modal-remarks').value=x.remarks||'';document.getElementById('modal-audit-section').hidden=false;document.getElementById('modal-audit-grid').innerHTML=auditSection(x);document.getElementById('modal-consumption-rows').innerHTML='';(x.consumptions||[]).forEach(row=>consumptionLine(row,false));if(!(x.consumptions||[]).length)consumptionLine({},false)}
async function openProductionModal(mode,id=null){if(mode==='create'){await loadProductionEditorForCreate();return}if(mode==='edit'){await loadProductionEditorForEdit(id);return}hideProductionEditor();activeMode=mode;activeId=id;let readonly=mode==='view';window.ErpModal.open({title:mode==='create'?'Add Production':mode==='edit'?'Edit Production':'Display Production',subtitle:'Production master and material consumption rows',size:'xl',body:productionModalBody(readonly),footer:readonly?'<button class="btn-erp" type="button" data-modal-close><i data-lucide="x"></i> Close</button>':'<button class="btn-erp" type="button" data-modal-close><i data-lucide="x"></i> Cancel</button><button class="btn-erp btn-primary" type="button" data-action="save-production"><i data-lucide="save"></i> Save Draft</button>'});const r=await axios.get(`${endpoint}/${id}`);const x=r.data.data;readonly=mode==='view'||x.status!=='Draft';document.getElementById('modal-status-label').innerHTML=badge(x.status);document.getElementById('modal-reference-label').textContent=x.posted_at?`Posted: ${x.posted_at}`:x.cancelled_at?`Cancelled: ${x.cancelled_at}`:'';document.getElementById('modal-production-no').value=x.production_no;document.getElementById('modal-production-date').value=x.production_date;document.getElementById('modal-production-time').value=(x.production_time||'').slice(0,5);document.getElementById('modal-bom-id').value=x.bom_id;document.getElementById('modal-produced-item-id').value=x.produced_item_id;document.getElementById('modal-team-id').value=x.team_id;document.getElementById('modal-fg-location-id').value=x.fg_location_id;document.getElementById('modal-produced-qty').value=x.produced_qty;document.getElementById('modal-production-cost').value=x.production_cost;document.getElementById('modal-labour-charge').value=x.labour_charge||0;document.getElementById('modal-remarks').value=x.remarks||'';document.getElementById('modal-audit-section').hidden=false;document.getElementById('modal-audit-grid').innerHTML=auditSection(x);document.getElementById('modal-consumption-rows').innerHTML='';(x.consumptions||[]).forEach(row=>consumptionLine(row,readonly));if(readonly){document.querySelectorAll('#modal-production-date,#modal-production-time,#modal-bom-id,#modal-produced-item-id,#modal-team-id,#modal-fg-location-id,#modal-produced-qty,#modal-production-cost,#modal-labour-charge,#modal-remarks').forEach(el=>el.disabled=true)}}
function modalPayload(){return{production_no:document.getElementById('modal-production-no').value,production_date:document.getElementById('modal-production-date').value,production_time:document.getElementById('modal-production-time').value,bom_id:document.getElementById('modal-bom-id').value,produced_item_id:document.getElementById('modal-produced-item-id').value,team_id:document.getElementById('modal-team-id').value,fg_location_id:document.getElementById('modal-fg-location-id').value,produced_qty:document.getElementById('modal-produced-qty').value,production_cost:document.getElementById('modal-production-cost').value,labour_charge:document.getElementById('modal-labour-charge').value,remarks:document.getElementById('modal-remarks').value,consumptions:[...document.querySelectorAll('#modal-consumption-rows tr')].map(tr=>({item_id:tr.querySelector('[name=item_id]').value,uom_id:tr.querySelector('[name=uom_id]').value,location_id:tr.querySelector('[name=location_id]').value,required_qty:tr.querySelector('[name=required_qty]').value,consumed_qty:tr.querySelector('[name=consumed_qty]').value,wastage_qty:tr.querySelector('[name=wastage_qty]').value,remarks:tr.querySelector('[name=remarks]').value}))}}
