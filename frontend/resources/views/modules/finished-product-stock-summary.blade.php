...
@section('title', 'Finished Product Stock Summary - Timber Inventory')
...
@section('content')
    ...
    <section class="erp-card summary-list-card">
        <div class="summary-card-head">
            <div>
                <h1>Finished Product Stock Summary</h1>
                <p class="text-muted">Current stock position for finished product items only.</p>
            </div>
...
<script>
const endpoint='/v1/finished-product-stock-summary';
...
async function boot(){const [items,types,locs]=await Promise.all([lookup('/v1/items'),lookup('/v1/material-types'),lookup('/v1/locations')]);const finishedItems=items.filter(item=>/^(finish|finished)\s+product/i.test(String(item.item_type||'')));item_id.innerHTML=opts(finishedItems,'item_id','item_name','All Finished Product Items');item_type.innerHTML='<option value="Finish Product" selected>Finish Product</option><option value="Finished Product">Finished Product</option>';material_type_id.innerHTML=opts(types,'material_type_id','material_type_name','All Types');location_id.innerHTML=opts(locs,'location_id','location_name','All Locations');colMaterialType.innerHTML=opts(types,'material_type_id','material_type_name','All');colLocation.innerHTML=opts(locs,'location_id','location_name','All');renderColumnPicker();applyColumnVisibility();load()}
