...
class StoreBOMRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'bom_no' => ['nullable', 'string', 'max:50', Rule::unique('bom_master', 'bom_no')->where('tenant_id', $user->tenant_id)->where('branch_id', $branchId)],
            ...
            'version_no' => ['required', 'string', 'max:20', Rule::unique('bom_master', 'version_no')->where('tenant_id', $user->tenant_id)->where('branch_id', $branchId)->where('finished_item_id', $this->input('finished_item_id'))],
